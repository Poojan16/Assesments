...
import { School, Users, GraduationCap, TrendingUp, Filter, Search, X, ChevronLeft, ChevronRight, Download, AlertCircle, Plus, BookOpen, Target, BarChart3, Award, TrendingDown, Calendar, Clock, Eye, Book, UserCheck, Activity } from 'lucide-react';
...
const AdminDashboard2 = () => {
  ...
  // Function to calculate school analytics
  const calculateSchoolAnalytics = (school) => {
    const schoolStudents = allStudents.filter(s => s.schoolId === school?.schoolId);
    const schoolTeachers = allTeachers.filter(t => t.schoolId === school?.schoolId);
    ...
    schoolStudents.forEach(student => {
      const className = classes.find(c => c.classId === student.classId)?.className || 'Unknown';
      ...
      classPerformance[className].totalScore += avgScore;
      if (avgScore >= 60) {
        classPerformance[className].passingStudents++;
      } else {
        classPerformance[className].failingStudents++;
      }
    });

    const classWiseData = Object.keys(classPerformance).map(className => ({
      name: className,
      avgScore: classPerformance[className].totalStudents > 0 
        ? classPerformance[className].totalScore / classPerformance[className].totalStudents 
        : 0,
      passingRate: (classPerformance[className].passingStudents / classPerformance[className].totalStudents * 100) || 0,
      students: classPerformance[className].totalStudents
    ...
    schoolScores.forEach(score => {
      ...
      subjectPerformance[subjectName].totalScores += score.score;
    ...
    const subjectWiseData = Object.keys(subjectPerformance).map(subjectName => ({
      name: subjectName,
      avgScore: subjectPerformance[subjectName].totalScores / subjectPerformance[subjectName].count,
      topScore: subjectPerformance[subjectName].topScore,
      lowScore: subjectPerformance[subjectName].lowScore,
      testsTaken: subjectPerformance[subjectName].count
    })).sort((a, b) => b.avgScore - a.avgScore);

    // Performance trends (simulated)
    const performanceTrends = [
      { name: 'Term 1', performance: school.avgScore * 0.9 },
      { name: 'Term 2', performance: school.avgScore * 0.95 },
      { name: 'Term 3', performance: school.avgScore },
      { name: 'Term 4', performance: school.avgScore * 1.05 }
    ];

    // Teacher-student ratio
    const teacherStudentRatio = schoolStudents.length > 0 
      ? (schoolTeachers.length / schoolStudents.length).toFixed(2) 
      : 0;
    ...
    const failDistributionData = [
      { name: 'Pass', value: passingStudents, color: '#10b981' },
      { name: 'Fail', value: schoolStudents.length - passingStudents, color: '#ef4444' }
    ];
    ...
    return {
      ...
      performanceTrends,
      teacherStudentRatio,
      failDistributionData,
      totalStudents: schoolStudents.length,
      totalTeachers: schoolTeachers.length,
      overallAvgScore: school.avgScore
    };
  };
  ...
  const filteredAndSortedSchools = useMemo(() => {
    let filtered = schools.filter(school => {
      ...
      const matchesZip = filterPincode === 'all' || String(school.pin) === filterPincode;

      return matchesSearch && matchesCity && matchesState && matchesZip;
  ...
  const paginatedSchools = filteredAndSortedSchools;
  ...
  const REQUIRED_TEMPLATE = {
    ...
    validations: {
      ...
      'Country': { type: 'string', pattern: /^[A-Za-z\s]+$/, required: true, allowedValues: ['India', 'USA', 'UK', 'Canada', 'Australia'] },
  ...
  const handleImport = (excelData) => {
    ...
    const url = `${backend_url}/importExcel`;
    const method = 'POST';
  ...
  const handleLinkConfig = async (value) => {
    if (value) {
      const expiryLink = await fetch(`${backend_url}/users/expiry?time=${Number(value)}`, {
        method: 'POST',
      })
      if (expiryLink.ok) {
  ...
  if (selectedSchool) {
    const analyticsData = calculateSchoolAnalytics(selectedSchool);
    ...
    // Enhanced school detail view with analytics
    return (
          ...
          <div className="bg-white rounded-xl shadow-sm p-4 md:p-6 mb-6">
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mt-6">
              <div className="bg-gradient-to-r from-blue-50 to-blue-100 p-4 rounded-lg border border-blue-200">
                ...
                <p className="text-2xl font-bold text-blue-800">{analyticsData.totalStudents}</p>
              ...
              <div className="bg-gradient-to-r from-green-50 to-green-100 p-4 rounded-lg border border-green-200">
                <div className="flex items-center gap-2 mb-2">
                  ...
                  <p className="text-sm text-green-700 font-medium">Teaching Staff</p>
                </div>
                <p className="text-2xl font-bold text-green-800">{analyticsData.totalTeachers}</p>
                <p className="text-xs text-green-600 mt-1">Teacher-Student Ratio: {analyticsData.teacherStudentRatio}</p>
              </div>
              ...
              <div className="bg-gradient-to-r from-purple-50 to-purple-100 p-4 rounded-lg border border-purple-200">
                ...
                <p className="text-2xl font-bold text-purple-800">{selectedSchool.avgScore?.toFixed(1)}%</p>
                <p className="text-xs text-purple-600 mt-1">
                  {selectedSchool.avgScore >= 80 ? 'Excellent' : selectedSchool.avgScore >= 70 ? 'Good' : 'Needs Improvement'}
                </p>
              </div>
              ...
              <div className="bg-gradient-to-r from-orange-50 to-orange-100 p-4 rounded-lg border border-orange-200">
                <div className="flex items-center gap-2 mb-2">
                  ...
                  <p className="text-sm text-orange-700 font-medium">Pass Rate</p>
                </div>
                <p className="text-2xl font-bold text-orange-800">
                  {analyticsData.totalStudents > 0
                    ? Math.round((analyticsData.failDistributionData[0].value / analyticsData.totalStudents) * 100)
                    : 0}%
                </p>
                <p className="text-xs text-orange-600 mt-1">
                  {analyticsData.failDistributionData[0].value} students passing
                </p>
              </div>
            </div>
            ...
            <div className="mt-6">
              <div className="flex border-b overflow-x-auto">
                <button
                  ...
                  className={`px-4 py-3 font-medium whitespace-nowrap ${analyticsTab === 'overall' ? 'border-b-2 border-blue-600 text-blue-600' : 'text-slate-600 hover:text-slate-800'
                    }`}
                >
                ...
                <button
                  onClick={() => setAnalyticsTab('classwise')}
                  className={`px-4 py-3 font-medium whitespace-nowrap ${analyticsTab === 'classwise' ? 'border-b-2 border-blue-600 text-blue-600' : 'text-slate-600 hover:text-slate-800'
                    }`}
                >
                ...
                <button
                  onClick={() => setAnalyticsTab('subjectwise')}
                  className={`px-4 py-3 font-medium whitespace-nowrap ${analyticsTab === 'subjectwise' ? 'border-b-2 border-blue-600 text-blue-600' : 'text-slate-600 hover:text-slate-800'
                    }`}
                >
              ...
              <div className="p-4">
                {analyticsTab === 'overall' && (
                  <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                    ...
                    {/* Pass/Fail Distribution */}
                    <div className="bg-white p-4 rounded-lg border border-slate-200">
                      <h3 className="font-semibold text-slate-800 mb-4">Pass/Fail Distribution</h3>
                      <div className="h-64">
                        <ResponsiveContainer width="100%" height="100%">
                          <PieChart>
                            <Pie
                              data={analyticsData.failDistributionData}
                              cx="50%"
                              cy="50%"
                              labelLine={false}
                              ...
                              dataKey="value"
                            >
                              {analyticsData.failDistributionData.map((entry, index) => (
                      ...
                      <div className="grid grid-cols-2 gap-4 mt-4">
                        <div className="text-center">
                          <div className="text-2xl font-bold text-green-600">{analyticsData.failDistributionData[0].value}</div>
                          <div className="text-sm text-slate-600">Passing Students</div>
                        </div>
                        <div className="text-center">
                          <div className="text-2xl font-bold text-red-600">{analyticsData.failDistributionData[1].value}</div>
                          <div className="text-sm text-slate-600">Failing Students</div>
                        </div>
                      </div>
                    </div>
                  </div>
                ...
                {analyticsTab === 'classwise' && (
                  <div>
                    <div className="mb-4">
                      <h3 className="font-semibold text-slate-800 mb-2">Class-wise Performance Analysis</h3>
                      <p className="text-sm text-slate-600">Average scores and pass rates by class</p>
                    </div>
                          ...
                          <BarChart data={analyticsData.classWiseData}>
                            ...
                            <Tooltip
                              formatter={(value, name) => {
                                if (name === 'avgScore') return [`${value.toFixed(1)}%`, 'Average Score'];
                                if (name === 'passingRate') return [`${value.toFixed(1)}%`, 'Pass Rate'];
                                return [value, name];
                              }}
                            />
                            <Legend />
                            <Bar dataKey="avgScore" name="Average Score" fill="#3b82f6" radius={[4, 4, 0, 0]} />
                            <Bar dataKey="passingRate" name="Pass Rate" fill="#10b981" radius={[4, 4, 0, 0]} />
                          </BarChart>
                        </ResponsiveContainer>
                      </div>
                    </div>
                      ...
                      {analyticsData.classWiseData.map((classData, index) => (
                        <div key={index} className="bg-slate-50 p-4 rounded-lg">
                          <div className="flex justify-between items-center mb-2">
                            <span className="font-semibold text-slate-800">Class {classData.name}</span>
                            <span className={`px-2 py-1 text-xs rounded-full ${classData.avgScore >= 70 ? 'bg-green-100 text-green-700' : 'bg-yellow-100 text-yellow-700'
                              }`}>
                          ...
                          <div className="text-sm text-slate-600">
                            ...
                            <div className="flex justify-between">
                              <span>Pass Rate:</span>
                              <span className="font-medium">{classData.passingRate.toFixed(1)}%</span>
                            </div>
                          </div>
                        </div>
                      ))}
                ...
                {analyticsTab === 'subjectwise' && (
                  <div>
                    <div className="mb-4">
                      <h3 className="font-semibold text-slate-800 mb-2">Subject-wise Performance Analysis</h3>
                      <p className="text-sm text-slate-600">Average scores, highest and lowest performance by subject</p>
                    </div>
                    <div className="bg-white rounded-lg border border-slate-200 overflow-hidden">
                      <div className="h-80">
                        <ResponsiveContainer width="100%" height="100%">
                          <BarChart data={analyticsData.subjectWiseData}>
                            ...
                            <Bar dataKey="topScore" name="Highest Score" fill="#10b981" radius={[4, 4, 0, 0]} />
                            <Bar dataKey="lowScore" name="Lowest Score" fill="#ef4444" radius={[4, 4, 0, 0]} />
                          </BarChart>
                        </ResponsiveContainer>
                      </div>
                    </div>
                    <div className="mt-4">
                      <h4 className="font-medium text-slate-800 mb-3">Subject Performance Summary</h4>
                      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
                        {analyticsData.subjectWiseData.map((subject, index) => (
                          <div key={index} className="bg-gradient-to-r from-slate-50 to-white p-3 rounded-lg border border-slate-200">
                            <div className="flex justify-between items-start mb-2">
                              <span className="font-semibold text-slate-800">{subject.name}</span>
                              <span className={`px-2 py-1 text-xs rounded-full ${subject.avgScore >= 70 ? 'bg-green-100 text-green-700' :
                                subject.avgScore >= 60 ? 'bg-yellow-100 text-yellow-700' :
                            ...
                            <div className="space-y-1 text-sm text-slate-600">
                              <div className="flex justify-between">
                                <span>Highest:</span>
                                <span className="font-medium text-green-600">{subject.topScore}%</span>
                              </div>
                              <div className="flex justify-between">
                                <span>Lowest:</span>
                                <span className="font-medium text-red-600">{subject.lowScore}%</span>
                              </div>
                              <div className="flex justify-between">
                                <span>Tests Taken:</span>
                                <span className="font-medium">{subject.testsTaken}</span>
                              </div>
                            </div>
                          </div>
                        ))}
          ...
          <div className="bg-white rounded-xl shadow-sm mb-6">
            <div className="flex border-b overflow-x-auto">
              ...
              >
                ...
                Teaching Staff ({analyticsData.totalTeachers})
              </button>
              <button
              ...
              >
                <GraduationCap className="inline mr-2" size={18} />
                Student Roster ({analyticsData.totalStudents})
              </button>
            </div>
        ...
        <div className="max-w-7xl mx-auto">
          ...
          <div className='flex items-center justify-between'>
            <div className="mb-6 md:mb-8">
              <h1 className="text-3xl md:text-4xl font-bold text-slate-800 mb-2">Education Analytics Dashboard</h1>
              <p className="text-slate-600">Comprehensive school performance monitoring and management system</p>
            </div>
          ...
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 md:gap-6 mb-6 md:mb-8">
            <div className="bg-gradient-to-br from-blue-50 to-blue-100 rounded-xl shadow-sm p-4 md:p-6 border-l-4 border-blue-500">
              <div className="flex items-center justify-between">
                <div>
            ...
            <div className="bg-gradient-to-br from-green-50 to-green-100 rounded-xl shadow-sm p-4 md:p-6 border-l-4 border-green-500">
              <div className="flex items-center justify-between">
                <div>
            ...
            <div className="bg-gradient-to-br from-purple-50 to-purple-100 rounded-xl shadow-sm p-4 md:p-6 border-l-4 border-purple-500">
              <div className="flex items-center justify-between">
                <div>
                  <p className="text-xs md:text-sm text-purple-600 font-medium">Total Teachers</p>
            ...
            <div className="bg-gradient-to-br from-orange-50 to-orange-100 rounded-xl shadow-sm p-4 md:p-6 border-l-4 border-orange-500">
              <div className="flex items-center justify-between">
                <div>
                  <p className="text-xs md:text-sm text-orange-600 font-medium">Avg Performance</p>
                  <p className="text-2xl md:text-3xl font-bold text-orange-800 mt-1">
          ...
          <div className="bg-white rounded-xl shadow-sm p-4 md:p-6 mb-6 md:mb-8">
            <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4 mb-6">
              <div>
                <h2 className="text-xl md:text-2xl font-bold text-slate-800">Schools Performance Dashboard</h2>
                <p className="text-slate-600 text-sm mt-1">Click on any school for detailed analytics</p>
              </div>
              ...
              <div className="flex flex-col sm:flex-row gap-3">
                ...
                <button onClick={() => navigate('/audit')} className="flex items-center gap-2 px-4 py-2 bg-purple-600 text-white rounded-lg hover:bg-purple-700">
                  <Activity size={18} />
                  Audit Trail
                </button>
              </div>
              ...
              <div className='flex items-center gap-4'>
                <div className="">
                  ...
                  <Select
                    ...
                    styles={customStyles}
                    className="w-full"
                    placeholder="Choose notification times..."
                  />
                </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 mb-6">
              {paginatedSchools.map(school => {
                const schoolStudents = allStudents.filter(s => s.schoolId === school?.schoolId).length;
                const schoolTeachers = allTeachers.filter(t => t.schoolId === school?.schoolId).length;
                const performanceColor = school.avgScore >= 80 ? 'green' : school.avgScore >= 70 ? 'blue' : 'orange';

                return (
                  <div
                    key={school.schoolId}
                    onClick={() => handleSchoolClick(school)}
                    className="bg-gradient-to-br from-white to-slate-50 rounded-xl p-4 cursor-pointer hover:shadow-lg transition-all transform hover:-translate-y-1 border border-slate-200 hover:border-blue-300 group"
                  >
                    <div className="flex items-center justify-between mb-3">
                      <div className="flex items-center gap-2">
                        ...
                        <span className={`px-2 py-1 text-xs rounded-full font-medium ${school.status === 'active'
                      ...
                      <div className={`px-2 py-1 text-xs rounded-full font-medium bg-${performanceColor}-100 text-${performanceColor}-700`}>
                        {school.avgScore?.toFixed(1) || 'N/A'}%
                      </div>
                    </div>
                    <h3 className="text-base font-bold text-slate-800 mb-2 line-clamp-2 min-h-[3rem] group-hover:text-blue-600 transition-colors">
                      {school.schoolName}
                    </h3>
                    <p className="text-xs text-slate-600 mb-3 truncate">{school.city}, {school.state}</p>
                    <div className="space-y-2">
                      ...
                      <div className="flex justify-between text-sm">
                        ...
                        <span className="font-semibold text-slate-800">{schoolStudents}</span>
                      </div>
                      <div className="flex justify-between text-sm">
                        <span className="text-slate-600">Teachers:</span>
                        <span className="font-semibold text-slate-800">{schoolTeachers}</span>
                      </div>
                      <div className="flex justify-between text-sm">
                        <span className="text-slate-600">Established:</span>
                        <span className="font-semibold text-slate-800">{school.established_year}</span>
                      </div>
                      <div className="mt-3 pt-3 border-t border-slate-200">
                        <div className="flex items-center justify-between text-xs">
                          <span className="text-slate-500">View Analytics</span>
