...
const ClassTeacherDashboard = () => {
    ...
    const fetchData = async () => {
      ...
      try {
        ...
        const [
          ...
          fetch(`${backend_url}/teachers/getWorkFlow?teacherId=${teacherId}`),
          fetch(`${backend_url}/teachers/class_and_subjects?teacherId=${teacherId}`),
          fetch(`${backend_url}/teachers/teacherSign/`),
          fetch(`${backend_url}/grades/`),
          fetch(`${backend_url}/admin/schools/${teacher?.schoolId}`)
        ]);
        ...
        try {
          ...
          const response = await fetch(
            `${backend_url}/teachers/getWorkFlow/ids?${params.toString()}`
          );
  ...
  const saveInlineScore = async (studentId, subjectId) => {
    ...
    try {
      ...
      const url = existingScore 
        ? `${backend_url}/students/score/${existingScore.studentScoreId}`
        : `${backend_url}/students/score`;
  ...
  const saveSignature = async () => {
    ...
    const saveSign = await fetch(`${backend_url}/teachers/teacherSign`, {
      method: 'POST',
      body: formData,
    });
  ...
  useEffect(() => {
    const handleSendReports = async () => {
      for (let i = 0; i < pdfData.allPdfs.length; i++) {
        ...
        const response = await fetch(`${backend_url}/teachers/uploadReport`, {
          method: 'POST',
          body: formData
        });
        const data = await response.json();
