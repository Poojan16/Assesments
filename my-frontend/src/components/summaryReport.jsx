...
//         const filteredStudents = sampleStudents?.data.filter(s => s.classId === sampleClasses?.data[0].classId);
//         setClassStudents(filteredStudents);
//         setAllStudents(sampleStudents?.data);
//         setScores(sampleScores?.data);
//         setClasses(sampleClasses?.data);
...
//     if (!selectedClass || !(scores).length) return 0;
...
//     return (total / (classScores).length).toFixed(2);
//   }, [selectedClass, scores, allStudents]);



//   // Calculate statistics for selected class
//   const stats = useMemo(() => {
//     if (!selectedClass || !(scores).length || !(classStudents).length) return { classAvg: 0, subjectAvgs: [], top10: [], needsAttention: [] };
...
//     const sorted = [...classStudents].sort((a, b) => b.overall - a.overall);
//     const top10 = sorted.slice(0, Math.min(10, sorted.length));
//     const needsAttention = sorted.slice(-Math.min(5, sorted.length));

//     return { classAvg,sorted, subjectAvgs, top10, needsAttention };
...
//     } else {
//       const idx = subjects.indexOf(sortBy);
//       return sorted.sort((a, b) => b.scores[idx] - a.scores[idx]);
//     }
//   }, [classStudents, sortBy, subjects]);
...
//               <p className="text-gray-600">Class: {selectedClassInfo?.className}</p>
//             </div>
...
//                     </p>
//                     <p className="text-sm text-gray-500">{stats.top10[0]?.overall}%</p>
//                   </div>
...
//                       <span className="font-bold text-indigo-600">{stats.subjectAvgs[idx]}%</span>
//                     </div>
//                     <div className="w-full bg-gray-200 rounded-full h-3">
//                       <div
//                         className={`h-3 rounded-full transition-all ${getGradeColor(stats.subjectAvgs[idx])}`}
//                         style={{ width: `${stats.subjectAvgs[idx]}%` }}
//                       ></div>
//                     </div>
//                   </div>
//                 ))}
...
//                         <th key={subject} className="text-center py-3 px-2 text-gray-600 font-semibold">{subject}</th>
//                       ))}
//                       <th className="text-center py-3 px-2 text-gray-600 font-semibold">Overall</th>
//                     </tr>
//                   </thead>
//                   <tbody>
//                     {stats.top10.map(student => (
...
//               <h2 className="text-xl font-bold text-red-600 mb-4">Students Needing Attention</h2>
...
//                     <p className="text-sm text-gray-600 mt-1">Attendance: {student.attendance}%</p>
//                   </div>
//                 ))}
//               </div>
//             </div>
...
//                 <div key={student.id} className="bg-white rounded-lg shadow-lg p-5 hover:shadow-xl transition">
...
//                         <span className="font-semibold">{student.scores[idx]}</span>
//                       </div>
//                     ))}
//                   </div>
